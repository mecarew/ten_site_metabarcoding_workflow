...
# Colour palette
site_cols <- brewer.pal(length(unique(plot_df$site)), "Spectral")
names(site_cols) <- unique(plot_df$site)

# -------------------------------------------------------
# PANEL A — Subsample proportion vs species richness
# -------------------------------------------------------
p1 <- ggplot(plot_df, aes(x = subsamp_prpn, y = sp_richness, color = site)) +
  geom_line(aes(group = site), linewidth = 0.6, alpha = 0.8) +
  geom_jitter(width = 0.01, height = 0.15, size = 2, alpha = 0.9) +
  scale_color_manual(values = site_cols) +
  scale_x_continuous(
    labels = scales::percent_format(accuracy = 1)  # show as percentage
  ) +
...
# -------------------------------------------------------
# PANEL B — Dry weight vs species richness
# -------------------------------------------------------
p2 <- ggplot(plot_df, aes(x = subsamp_dw, y = sp_richness, color = site)) +
  geom_line(aes(group = site), linewidth = 0.6, alpha = 0.8) +   # join dots
  geom_jitter(width = 0.15, height = 0.15, size = 2, alpha = 0.9) +
  scale_color_manual(values = site_cols) +
  scale_y_continuous(limits = c(0, NA)) +     # y starts at 0
  labs(
    x = "dry weight (g)",
    y = "species richness"
  ) +
  ...
  labs(color = "site")  # set the legend title

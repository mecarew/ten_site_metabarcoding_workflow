...
directory_path <- here::here("results/import_data")  # Replace with your folder path
...
# Filtering checks
```{r}
##checking filering_asvs that are kept after filtering
short_curated_otus <- short_lulu$curated_otus
short_curated_otus_df <- if (!is.data.frame(short_curated_otus)) {
short_curated_otus <- as.data.frame(short_curated_otus)
}

names(short_curated_otus_df)[names(short_curated_otus_df) == "short_curated_otus"] <- "all_filter_asvs"

right_curated_otus <- right_lulu$curated_otus
right_curated_otus_df <- if (!is.data.frame(right_curated_otus)) {
right_curated_otus <- as.data.frame(right_curated_otus)
}

names(right_curated_otus_df)[names(right_curated_otus_df) == "right_curated_otus"] <- "all_filter_asvs"


all_curated_otus <- rbind(right_curated_otus_df, short_curated_otus_df)


write.csv(all_curated_otus, file = here("results/MiSeq17_all_curated_asvs_filtered_008.csv"), row.names = FALSE)

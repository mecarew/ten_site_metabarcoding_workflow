...
load(here::here("results/cache/01.RData"))
```
...
First, define the lengths that amplicon groups will be truncated to. Set truncation length of reads based on quality plots in the 01_preprocessing. Where does read quality drop off

```{r}
truncation_lengths <- data.frame(
  SampleName = c("short", "right"), # customize amplicon name for dataset
  forward_trunc_length = c(140, 140), # set truncation length for forward reads based on quality plots
  reverse_trunc_length = c(140, 200) # set truncation length for reverse reads based on quality plots
)
...
```{r}
# save denoising statistics
short_denoising_stats <- read_qza(here("results/dada2/short_denoising_stats.qza"))$data
write.csv(short_denoising_stats,file.path(statistics_dir, "short_denoising_stats.csv"), row.names = FALSE)

right_denoising_stats <- read_qza(here("results/dada2/right_denoising_stats.qza"))$data
write.csv(right_denoising_stats,file.path(statistics_dir, "right_denoising_stats.csv"), row.names = FALSE)
```
...
directory_path <- here::here("results/import_data") 

# List all items in the directory
items <- list.files(directory_path, full.names = TRUE)

# Filter for directories (samples)
folders <- items[dir.exists(items)]

# Count the number of sample folders
num_samples <- length(folders)
cat("Number of samples in the MiSeq run:", num_samples, "\n")

# Calculate p_prevalence

# Minimum number of samples an ASV must appear in
min_samples <- 2

# Calculate prevalence proportion
p_prevalence <- min_samples / num_samples
cat("Set 'p_prevalence' to:", p_prevalence, "\n")

# Set p_abundance
# Minimum relative abundance to remove singletons (e.g., 2 reads in smallest sample)
# If you know your smallest sample size, you can calculate:
# min_reads <- 2
# smallest_sample_size <- 10000  # Replace with actual smallest sample size
# p_abundance <- min_reads / smallest_sample_size

# For now, set a conservative default for singletons
p_abundance <- "0.00001"  # 0.001% relative abundance
cat("Set 'p_abundance' to:", p_abundance, "\n")
...
before_df <- as.data.frame(before_filtering$data)
after_df  <- as.data.frame(after_filtering$data)

# Summarize each ASV: total reads and prevalence (# of samples with >0)
summarize_asvs <- function(df) {
  tibble(
    ASV = rownames(df),
    total_reads = rowSums(df),
    prevalence = rowSums(df > 0)
  )
}

before_summary <- summarize_asvs(before_df) %>% 
  mutate(status = "Before Filtering")

after_summary  <- summarize_asvs(after_df)  %>% 
  mutate(status = "After Filtering")

# Combine
combined <- bind_rows(before_summary, after_summary)

# Create plot object
p <- ggplot(combined, aes(x = prevalence, y = total_reads, color = status)) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0.6) +
  scale_y_log10() +
  labs(
    x = "Prevalence (number of samples)",
    y = "Total reads per ASV (log10)",
    title = "Effect of Filtering on ASVs"
  ) +
  theme_minimal() +
  scale_color_manual(values = c("Before Filtering" = "red",
                                "After Filtering" = "blue"))

# Save plot
ggsave("ASV_filtering_effect.png", plot = p, width = 8, height = 6, dpi = 300)

# Save plot
ggsave(file.path(statistics_dir,"short_asv_filtering_effect.png"), plot = p, width = 8, height = 6, dpi = 300)
...
before_summary <- summarize_asvs(before_df) %>% mutate(status = "Before Filtering")
after_summary  <- summarize_asvs(after_df)  %>% mutate(status = "After Filtering")

# Combine
combined <- bind_rows(before_summary, after_summary)


# Plot: total reads vs prevalence
ggplot(combined, aes(x = prevalence, y = total_reads, color = status)) +
  geom_jitter(width = 0.2, height = 0.2, alpha = 0.6) +
  scale_y_log10() +  # log scale to see low-abundance ASVs clearly
  labs(
    x = "Prevalence (number of samples)",
    y = "Total reads per ASV (log10)",
    title = "Effect of Filtering on ASVs"
  ...
  theme_minimal() +
  scale_color_manual(values = c("Before Filtering" = "red", "After Filtering" = "blue"))

# Save plot
ggsave(file.path(statistics_dir,"right_asv_filtering_effect.png"), plot = p, width = 8, height = 6, dpi = 300)

...
past_macros <- past_macros %>%
  ...
  dplyr::filter(year >= 2010 & year <= 2021)           # keep years 2015 to 2020
```


```{r}
# read in metabarcoding data
metab_filt <- read.csv(paste0(dir,"results/ten_site_data_summaries/ten_sites_final_long_format_16_12_2025.csv")) %>%
  mutate(year = 2021)
...
nmds_plot <- ggplot(scores_df, aes(NMDS1, NMDS2, color = treatment, shape = site)) +
  ...
  theme(
    ...
    axis.title.x = element_text(hjust = 0.5), # left-align x-axis label
    axis.title.y = element_text(hjust = 0.5), # left-align y-axis label

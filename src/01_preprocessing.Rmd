...
```{r}
samples_df <- read.table(here::here("data/metadata/samples.tsv"), header=TRUE, stringsAsFactors=FALSE)
...
```{r}
# save cutadapt stats for record
statistics_dir <- file.path(results_dir, "statistics")
mkdir(statistics_dir)
write.csv(trim_stats_df,file.path(statistics_dir, "cutadapt_statistics.csv"), row.names = FALSE)
```
...
```{r}
max_count <- 10000
all_plots <- list()  # store combined plots per amplicon group
...
for (amplicon_group in amplicon_groups) {
  ...
  g_forward <- ggplot(forward_df, 
    ...
    theme_bw() +
    labs(title=sprintf("Forward reads: %s", amplicon_group),
         x="Sequence Base", y="Median Quality Score") +
    theme(plot.title = element_text(size=6))
  ...
  g_reverse <- ggplot(reverse_df, 
    ...
    scale_alpha_continuous(limits=c(0, 1)) +
    theme_bw() +
    labs(title=sprintf("Reverse reads: %s", amplicon_group),
         x="Sequence Base", y="Median Quality Score") +
    theme(plot.title = element_text(size=6))
  
  # Place forward and reverse side by side
  combined <- g_forward | g_reverse  # "|" stacks horizontally
  all_plots[[amplicon_group]] <- combined
}

# Stack all amplicon groups vertically
final_plot <- wrap_plots(all_plots, ncol = 1)  # 1 column: each row = 1 amplicon group

# Save to PNG
ggsave(filename = file.path(statistics_dir, "all_amplicons.png"),
       plot = final_plot,
       width = 12,   # wider to accommodate side-by-side
       height = 3 * length(amplicon_groups))  # scale height per number of groups
...
```{r}
mkdir(here("results/cache"))
save.image(here("results/cache/01.RData"))
```

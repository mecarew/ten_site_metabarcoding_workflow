...
This notebook examine the output data for the Miseq run
...
```{r}
# input unfiltered metabarcoding data with final taxonomy
metab_fil <- read.csv(here::here("results/ten_site_data_summaries/ten_sites_vsearch_data_summary_filled_long_format_24_02_2026.csv"))

metab_fil <- metab_fil[!grepl("^Cont", metab_fil$sample), ]

# input final flitered metabarcoding data     
...
dir.create(
  here::here("results/data_analysis/run_stats"),
  recursive = TRUE,
  showWarnings = FALSE
)
...
writexl::write_xlsx(
  metab_fil_reads_tab,
  path = here::here("results/data_analysis/run_stats/DNA_metabarcoding_reads_summary.xlsx")
)
...
print(paste("percentage_overlap_btn_PCR_replicates_unfiltered =", asv_percentage_overlap))
```
...
print(paste("asv_percentage_overlap_btn_PCR_replicates_filtered =",asv_percentage_overlap))
```
...
print(paste("no. of families = ", fam_count))
```

...
```{r}
# read in flies
asv_lib <- read.csv(here::here("data/asv_library_Dec_2025.csv"))
```
...
# remove low frequency detections < 2 reads
```{r}
metab_long_hf <- metab_long %>%
  filter(reads >= 2 )

metab_long_lf <- metab_long %>%
  filter(reads < 2)  # Corrected this line
...
cat("Number of records with reads >= 2:", hf_count, "\n")
cat("Number of records with reads < 2:", lf_count, "\n")
```
...
sp_removed_metab_long_with_species <- filtered_dataframe_top_sp %>%
  left_join(counts_sp, by = c("site", "species")) %>%
  filter(count <= 1 | is.na(count))

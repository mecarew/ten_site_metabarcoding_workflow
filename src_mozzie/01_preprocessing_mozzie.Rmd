...
results_dir <- here::here("metabarcodong_workflow/results")

# Log files will be stored inside the logs directory inside the results directory
logs_dir <- file.path(results_dir, "metabarcodong_workflow/logs")
```
...
slurm_conffile <- here::here("metabarcoding_workflow/src_mozzie/.batchtools.conf.R")
slurm_template <- here::here("metabarcoding_workflow/src_mozzie/batchtools.slurm.tmpl")
...
```{r}
samples_df <- read.table(here::here("metabarcoding_workflow/data/metadata/samples.tsv"), header=TRUE, stringsAsFactors=FALSE)
